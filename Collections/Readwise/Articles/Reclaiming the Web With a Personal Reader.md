# Reclaiming the Web With a Personal Reader

![rw-book-cover](https://olano.dev/assets/img/feedi1.png)

## Metadata

- Author: [[Facundo Olano]]
- Full Title: Reclaiming the Web With a Personal Reader
- Category: #articles
- Summary: The author of the article describes their experience with burnout and disillusionment with the software industry. They turned to Mastodon, an alternative to Twitter, and discovered a community of people seeking a more open and independent web. However, they found that Mastodon did not meet their needs as an information hub. They then decided to build their own personal feed reader, prioritizing ease of use and the user experience. They chose to use Python and created a classification system to sort the feed by frequency of updates, ensuring that they saw the least frequent sources first.
- URL: https://olano.dev/2023-12-12-reclaiming-the-web-with-a-personal-reader/

## Full Document

[[Full Document Contents/Articles/Reclaiming the Web With a Personal Reader.md|See full document content →]]

## Highlights

- Last year I experienced the all-too-common career burnout. I had a couple of bad projects in a row, yes, but more generally I was disillusioned with the software industry. There seemed to be a disconnection between what I used to like about the job, what I was good at, and what the market wanted to buy from me. ([View Highlight](https://read.readwise.io/read/01hjrn3p0my61z9k3we098m471))
