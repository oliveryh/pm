# Finding a New Software Developer Job

![rw-book-cover](https://henrikwarne1.files.wordpress.com/2024/01/dsc_2037-3938421749-e1706351475537.jpg)

## Metadata
- Author: [[Henrik Warne's blog]]
- Full Title: Finding a New Software Developer Job
- Category: #articles
- Summary: The author shares their experience of being laid off and searching for a new software developer job. They were in contact with 30 companies, received rejections from 8, no response from 6, and offers from 3. They utilized LinkedIn, job ads, and other sources to find job opportunities. The author discusses their interview process, preparations, coding tests, and salary negotiations. Ultimately, they provide insights and lessons learned from their job search experience.
- URL: https://henrikwarne.com/2024/02/11/finding-a-new-software-developer-job/

## Full Document
[[Full Document Contents/Articles/Finding a New Software Developer Job.md|See full document content →]]

## Highlights
- **Applying to known companies.** Next, I went through my list of companies I would like to work for, and looked to see if they had any open developer roles. I first looked on the company page on LinkedIn, then clicked on the Jobs tab. Many of the companies were actively recruiting. A good thing when you click on an ad is that you can see how many people have applied, and how old the ad is. Sometimes I would also go to the company home page and look at their career page. But I found it convenient to go through LinkedIn, where the format is the same, and you can see if any of your contacts work there. Sometimes you can also see who has posted the ad. ([View Highlight](https://read.readwise.io/read/01hzcvc3p4x344v9g75crbb5n5))
- Since before, I had a Word document that I called *Interview Tips*. In it I wrote down things to think about before an interview, in a format that is easy to review quickly. One section I added now was **Behavioral Questions**. These are questions of the form *“Tell me about a time when you disagreed with a coworker. How did you resolve it?”*. Typically, you should answer them using the STAR framework: Situation, Task, Action, Result, Reflection. In the past, I have failed interviews because of these questions – I hadn’t prepared, and couldn’t come up with good examples on the spot in the interviews. ([View Highlight](https://read.readwise.io/read/01hzcvq5k2exm8zjdx2j8wd349))
- I also watched a good video by Gergely Orosz, [Confessions from a Big Tech Hiring Manager: Tips for Software Engineering Interviews](https://www.youtube.com/watch?v=vFOw_m5zNCs), on technical interviews in general. Some takeaways: be curious and collaborative, and ask questions. ([View Highlight](https://read.readwise.io/read/01hzcvrd97z9dqxhe0r3k719g9))
