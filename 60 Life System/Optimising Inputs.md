# Resources

- [Optimizing Your Inputs | Colossus®](https://www.joincolossus.com/episodes/85214124/srinivasan-optimizing-your-inputs?tab=blocks)
- [How to stop being "terminally online" | Hacker News](https://news.ycombinator.com/item?id=32374207)

# Methodology

1. List every concept/object/process I spend time/energy/money on
2. Assess value, urgency, and cost
3. Remove low-value or high-cost items; make time for what matters

- **Document** — map inputs (digital in-tray), actions (internet habits), and outputs (created/archived work)
- **Reduce** — cut the number of inputs; uninstall apps, unsubscribe from feeds
- **Consolidate** — funnel what remains into a single stream with minimal distractions
- **Understand motivations** — avoid maintaining systems for their own sake
- **Maintain** — do regular check-ins to keep the system honest

Some examples of inefficiencies include:

- Food shopping multiple times per week
- Going through emails
- Checking my monthly budget
- Reading news websites (BBC, Hacker News)
- Writing To Do lists which are most likely repeats of information that I have stored elsewhere
- Reading about something that has no clear influence on my life at all
- Doing chores late on a Sunday instead of winding down and relaxing
- Watching TV in general

# Build Tools for Workflows

https://thesephist.com/posts/tools/

Tools we consume will largely fall short of what we expect. When others others build tools for us to use, they either design tools after their own workflows and mental models, or design for a mass market.

Instead of trying to fit our workflows to fit around the tools available to us, we should be designing tools to fit around our workflows.

Not only this, but the investment in time is worth it. I was able to build a task app that fixed my failed attempts of trying to fit my workflow around mass marketed tools.

I'm not able to consolidate all the apps I once used, into tools that adapt to my workflow. The apps that I build, along with the highly customizable tools (Notion & Obsidian).

The other benefit of writing your own tools is ownership and longevity. Obsidian is stored in Markdown, a format that will be supported for years to come.

# Concepts

- 3D Printer
- Alerts
- Amsterdam
- Analogue solutions to modern problems
- APIs
- Archiving
- Articles
- Automation
- Back mobility
- Backlog
- Backups
- Beer with Friends
- Belonging maintenance
- Belongings
- Birthdays
- Blog
- Bookmarked Websites
- Books
- Bread and Bread Making
- Budget
- Bullet Journal
- Buy birthday presents
- Buying and selling stuff
- Buying things online
- Calendar
- Calisthenics
- Calligraphy
- Car Insurance
- Cardio
- Career
- Cats and Dogs
- Charts
- Chat Ops
- Chats
- Checklists
- Classical Music
- Clean bathroom
- Clean email
- Clean kitchen
- Clean Living
- Clean Solutions
- Cleaning
- Clip nails
- CLIs
- Code
- Coffee
- Community
- Computer Graphics
- Contacts
- Cooking
- Core Physical Ability
- Credit Rating
- Cron jobs
- Cycling
- Cycling routes
- Dancing
- Dashboards
- Data Visualisation
- Databases
- Dental
- Dictation
- Diet
- Digital accounts
- Digital Storage
- Dinner Parties
- Domains
- Donations
- Drawing
- Drinking 2L+ water a day
- Eating a balanced diet
- eBooks
- Email
- Environmentalism
- Essentialism
- Exercise classes
- Exercises / Workouts
- Family
- Favourite beers
- Favourite brands
- Favourite places
- Favourites
- Finances
- Food deliveries
- Food shopping
- Forecasts
- Game Nights
- Gardening
- GERD treatment
- Gym
- Habits
- Hacker News
- Hammocks
- Hard drive
- Health Insurance
- Hiking
- Holidays
- Home automation
- Hoovering
- Hydration
- Inventory
- Invest money
- Investments / ISA
- Journaling
- Kidney function recovery
- Kobo reader project
- Language
- Launcher
- Laundry
- Letters
- Life Insurance
- Listen to music
- Macros
- Make notes on Articles
- Making lists
- Maps
- Meal plans
- Meaningful Movies
- Medical
- Meditation
- Meeting people
- Mental Health
- Mentoring
- Messaging
- Mid Century Furniture
- Minimalism
- Mobility
- Moisturize
- Monitor finances
- Monitoring
- Mortgage
- Mountains
- Muscle Pain
- Music
- Nature
- Navigation
- Neat code solutions
- Networking
- News
- Newsletters
- Notebooks
- Notifications
- Notion
- Nutrients
- Nutrition
- Observation
- Obsidian
- Offline Maps
- Old Stuff
- Online accounts
- Online Videos
- Open Source Contributions
- Optician
- Optimization
- Packing lists
- Passwords
- Pay utitilies
- Paychecks
- Pension
- People
- Personal Banking
- Personal coding projects
- Phone calls
- Photography
- Photos
- Physio
- Piano
- Pilates
- Planning cycling routes
- Plants
- Playing XBOX
- Podcasts
- Points of Interest
- Projects
- Quantified Self
- Quiet
- Reading
- Reading the News
- Receipts
- Recipes
- Recommendations
- Recycling
- Reddit
- Reminders
- Renovation
- Repairing something that's broken
- Repairs
- Repot plants
- Restaurants
- Routines
- RSS feeds
- Running
- Screen Time
- Selling things online
- Sensors
- SEO
- Servers
- Share Options
- Shave
- Shopping for new things
- Shopping lists
- Skin Care
- Social media
- Socialising with housemates
- Sort posessions
- Sourdough bread
- Sports
- Standing desk
- Strength training
- Subscriptions
- Swimming
- Take out bins
- Tasks
- Taxes
- Tennis
- Things to Do
- Thinking
- Time Tracking
- Tiny Houses
- Tracking diet
- Tracking finances
- Travel
- TUIs
- Turkish Cuisine
- TV / Film
- UIs
- Vacation Days
- Veganism
- Video Journalling
- Visiting friends
- Visiting home
- Visiting restaurants to eat at
- Vouchers
- Walking
- Walking routes
- Washing the kitchen
- Watching Movies
- Watching TV
- Water plants
- Weather
- Weight
- Wine
- Work
- Writing
- X
- Yoga
- YouTube videos

# Log

## 2026-03-21

- MyMind
- https://sublime.app/


A lot of modern life now means interacting with devices or online accounts. Even if they don't, we can often augment our experiences with help from digital resources.

Specific data flows that may need improving/removing etc.

- Obsidian
	- Daily notes creation
- iPhone
	- Journal app

On a given day, I can think of these categories of things to do

- Habit (Things I want to do almost every day of the week)
- Routines (Things that I want to do regularly)
- One-off tasks (Smaller tasks that don't take too much planning to complete)
- Projects (Things that take more thought and time to complete. Only a few projects can be completed in a sprint)
- Hobbies (Things that I enjoy doing on a regular basis and are generally wholesome)

## 2025-06-01

These few months I've obsessed over web technologies and automation. I've spent 5 days thinking entirely about it. The plants around me have gone dry, the floor strewn with clothes and dirty plates. I've added things to a virtual todo list and I've not really asked myself why I want to create the most efficient, lightweight, rapidly developed website.

My world isn't just my screen. It's how I nourish myself, the relationships I cultivate, and the experiences I seek out. This has gone too far and I need to take a break.

## 2025-05-01

I still feel like my digital life is now too big to control. It's like 10 years of digital hoarding is looking back at me when I try to do anything. I think COVID must have been when things got particularly bad. I wonder if I need to apply some minimalism principles.

## 2025-04-05

- I was looking for an app to categorise photos with tags and make them easily searchable
	- Open source app built by a 14yo  https://apps.apple.com/gb/app/memento-your-mental-inbox/id6541765296
	- Memento Database https://apps.apple.com/gb/app/memento-database/id1583610478 - An app I've used in the past
	- Slide box https://apps.apple.com/gb/app/slidebox-photo-cleaner-app/id984305203
	- Karakeep

## 2025-04-04

I've noticed that my time away from work is becoming more and more of a reminder of how I _should_ be spending my time. I think I'm a bit of a workaholic in some ways, and I class any form of software engineering as "work" because it's effectively the exact same thing. I'd like to develop a gust of life, but am often overwhelmed by the perceived complexity of doing everything properly, and planning it all effectively. I won't feel like I can go and enjoy myself because IO'm painfully aware of all the responsibilities of maintaining a house.

In reality, there will always be more maintenance that could be done, so it's important to balance this all with me enjoying myself.

## 2025-04-01

How do I know what to spend my time on. I'd like to prioritise reading, exercise, meeting people, but I end up filling my time with coding, chores, and staring at my laptop. A part of it is a need to feel productive, and unfortunately there appears to be a limitless number of tasks I could be doing at home and work.
